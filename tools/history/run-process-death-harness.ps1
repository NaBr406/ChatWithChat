...
$TestClass = "cn.nabr.chatwithchat.data.history.ChatHistoryProcessDeathInstrumentedTest"
...
try {
    ...
    Write-Host "`n== Phase one: persist the history queue and kill the process =="
    $PhaseOne = Invoke-AdbCaptured -CommandArguments @(
        ...
        "-e",
        "class",
        "$TestClass#phase1_persistQueuedWorkAndKillProcess",
    ...
    Write-Host "`n== Phase two: restart and drain the durable history queue =="
    $PhaseTwo = Invoke-AdbCaptured -CommandArguments @(
        ...
        "-e",
        "class",
        "$TestClass#phase2_reopenAndDrainQueuedWorkAfterProcessDeath",
        $InstrumentationComponent
    )
    ...
    Write-Host "`nHistory process-death harness passed on $DeviceSerial."
    Write-Host "Verified: durable history queue persisted before process death and drained after instrumentation restart."
} finally {
    Pop-Location
}
